...
The main executable is a single bash script at `./wt` with no dependencies beyond git, bash, and optional tools (fzf for interactive mode, jq for updates).
...
2. **Version Management**: VERSION constant is hardcoded at wt:7 and must be manually updated. The release workflow (`.github/workflows/release.yml`) validates that the version in the script matches the git tag before releasing.
...
4. **Worktree Creation**: The `add_worktree()` function (wt:39-104) creates new worktrees in the parent directory of the main repository. It intelligently handles:
...
1. Validates version in script matches git tag (wt:20-22 in script)
...
2. **Shell Wrappers Required**: Users must run `wt init <shell> | source` to create the wrapper function that handles directory changes. The init functions are defined at wt:120-157.
...
6. **Error Handling**: Script uses `set -e` (wt:2) so any command failure will exit. Interactive mode validates fzf is installed (wt:23-31).
